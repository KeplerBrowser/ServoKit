...
| Favicon notifications | Native Rust event | Available on Servo-backed Rust paths; not exposed through React Native or the C event bridges | Low | `HostEvent::FaviconChanged` forwards Servo's per-webview callback as an owned copy of the first image frame. ServoKit does not fetch icons or synthesize navigation clears. |
...
| Exportable offscreen GPU frames | Surface/compositor mode | Implemented on macOS | Medium | `OffscreenSurface::exportable()` publishes retained opaque 32BGRA `CVPixelBuffer` frames with exact completion; Windows shared textures and Linux dma-buf remain backlog |
