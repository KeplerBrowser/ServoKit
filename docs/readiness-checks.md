...
| Windows | `.github/workflows/windows-host-lifecycle.yml` runs `scripts/windows-host-lifecycle.ps1`, which records commit, worktree status plus tracked diff summary/name-status relative to `HEAD`, source file hashes, exact command manifest, submodule, PowerShell, Windows, Visual Studio C++ x64 discovery, x64 MSVC Rust/toolchain, and Cargo/Servo dependency evidence, then runs `cargo test --manifest-path crates/Cargo.toml -p servokit-host-desktop --test c_boundary --release --locked` and `cargo check --manifest-path examples/desktop-winit/Cargo.toml --locked` on `windows-latest` with `CARGO_BUILD_TARGET=x86_64-pc-windows-msvc`. The same script may be run on a prepared Windows Rust/Servo workstation. The React Native Windows source project lives in `packages/react-native-servokit/windows` and depends on that desktop host staticlib evidence before runtime acceptance. `.github/workflows/windows-rnw-source.yml` and `scripts/windows-rnw-source.ps1` record the #6 source-build slice: desktop staticlib build, RNW `react-native config` autolinking proof for the example app, RNW codegen drift check against the checked-in generated component header, and MSBuild logs for the source project. | From an attended Windows x64 desktop session, run `scripts/windows-desktop-smoke.ps1 -Attended`. It starts the fixture server, runs `examples/desktop-winit` with `--smoke`, captures stdout, and requires a native Windows window to render the smoke fixture while output shows load completion plus `smoke action=input-probe`, `input_probe=complete`, `smoke action=reattach-cycle`, `surface_attach_count=2`, `surface_detach_count=2`, and `reattach=complete`. Then use interactive mode to confirm resize, DPI changes, native focus, URL/load, real pointer/wheel, keyboard/text input, navigation, close/detach, and teardown behavior. For RNW, first run `scripts/windows-rnw-source.ps1` from a prepared Windows x64 checkout, then run `scripts/windows-rnw-smoke.ps1 -Attended -AppRoot <rnw-app-root>` against a RNW app with a generated Windows project and app source that loads the smoke fixture URL and exposes the required event/control/recycle hooks, then record visible rendering, resize/DPI, hide/show, pointer/wheel, keyboard/text, focus, shared navigation commands/events, two recycle/remount cycles, teardown, and stale-handle behavior. | `NativeChildSurface` through the `winit` / `raw-window-handle` path and, for RNW, the package-private desktop C boundary behind a C++/WinRT Fabric component. Keep Windows-specific details target-gated and expressed through existing ServoKit facade vocabulary. | Windows RNW is source-side only and not runtime-accepted or packaged. GUI smoke requires an interactive desktop session. `GpuLayerSurface` / shared-D3D export is not implemented. |
...
## Windows host lifecycle workflow

The Windows host lifecycle workflow is the durable issue #5 automation slice for
the target-gated Win32 mapping. It is intentionally narrower than React Native
Windows and packaging work: it runs `scripts/windows-host-lifecycle.ps1`, which
resolves the Visual Studio C++ x64 developer shell with `vswhere`, records
commit, worktree status plus tracked diff summary/name-status relative to
`HEAD`, source file hashes, the exact command manifest, submodule, PowerShell,
Windows, Rust, Cargo, rustup, clang, and MSVC-linker evidence for
`x86_64-pc-windows-msvc`, captures Cargo metadata and the resolved Servo package
version, runs the desktop private C-boundary test, runs the
`windows_native_lifecycle.cpp` HWND
attach/resize/detach/reattach/stale-token harness through that test, and checks
the `desktop-winit` proof app. It uploads the command logs and metadata as
`ServoKit-Windows-host-lifecycle-evidence`. It does not claim React Native
Windows, binary distribution, GPU surfaces, ARM64 Windows, or new public
controller capabilities.

The workflow does not claim attended GUI smoke. Before closing issue #5 or any
Windows host proof ticket, still run `examples/desktop-winit` in an interactive
Windows x64 desktop session against
`http://127.0.0.1:8481/smoke/index.html` and record the tested Windows, Rust,
Servo, and toolchain versions with the visible rendering, resize/DPI, input,
navigation, teardown, and stdout event evidence. For synthetic input plus
detach/reattach evidence, run smoke mode and record the `smoke
action=input-probe`, `input_probe=complete`, `smoke action=reattach-cycle`,
`surface_attach_count=2`, `surface_detach_count=2`, and `reattach=complete`
output. The repeatable attended smoke wrapper is:

```powershell
.\scripts\windows-desktop-smoke.ps1 -Attended -EvidenceDir $env:TEMP\servokit-windows-desktop-smoke
```

On a prepared Windows x64 workstation, run the same headless evidence slice with:

```powershell
.\scripts\windows-host-lifecycle.ps1 -EvidenceDir $env:TEMP\servokit-windows-evidence
```
