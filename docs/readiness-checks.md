...
## Cold profile branch proof

Run the proof on an interactive desktop session:

```sh
cargo run --locked --manifest-path examples/desktop-winit/Cargo.toml \
  --bin profile-branch-proof -- prove
```

The command builds one executable from one checked-in Cargo root and lockfile.
That executable starts a controlled loopback fixture, uses a visible native
ServoKit process to seed an opaque `HttpOnly` test cookie plus a known
`localStorage` value, shuts Servo down, and captures the closed
config directory as immutable revision `R1`. It copies `R1` into distinct local
and cloud branches, then keeps a visible local ServoKit process and a
software-rendered headless ServoKit process alive concurrently until the server
has observed authenticated, independently diverged state from both. Finally it
reopens each branch in another fresh headless ServoKit process and checks that
authentication and its branch-specific `localStorage` value survived.

A passing run prints the canonical revision and branch paths, the unchanged
SHA-256 content digest for `R1`, clean child-process exits,
`concurrent_authenticated=true`, `fresh_process_reopen=true`, and
`result=PASS`. Disposable files are removed after success and preserved with an
explicit path after failure. The fixture reports only whether its opaque test
cookie was present; the proof never prints or interprets Servo's cookie,
local-storage, or other profile files.

This is an attended, proof-only executable, not a public profile API or cloud
service. It does not cover IndexedDB relocation, hot checkpointing, live
directory sharing, branch merging, real credentials, encryption/upload, leases,
remote transport, or production agent orchestration.
...
| cold profile branch proof | Prove a terminally closed Servo config directory can become immutable `R1`, fork into two independent branches, authenticate concurrently in visible and headless ServoKit processes, diverge, and reopen with isolated state. | Compile the single locked proof binary:<br>`cargo check --locked --manifest-path examples/desktop-winit/Cargo.toml --bin profile-branch-proof` | `cargo run --locked --manifest-path examples/desktop-winit/Cargo.toml --bin profile-branch-proof -- prove` | The loopback server observes the opaque test authentication in both concurrent branches; `localStorage` preserves `R1`, diverges to `local`/`cloud`, and retains those values after fresh-process reopen. The immutable revision digest is unchanged and every Servo process exits cleanly. | Requires an interactive desktop session because seed and local runners intentionally use visible native ServoKit surfaces. IndexedDB relocation is not covered. This is cold branching evidence, not live synchronization, credential export, or a production profile API. |
